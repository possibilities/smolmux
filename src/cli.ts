...
export function parseArgs(args: string[]): CliOptions {
  const options: CliOptions = { foreground: false, localOnly: false, help: false, version: false, command: null, name: "default" }
  ...
  if (positional.length === 0) {
    if (options.foreground || options.localOnly) throw new UsageError("--foreground requires start; --local-only requires doctor")
    return options
  }
  ...
  if (options.foreground && command !== "start") throw new UsageError("--foreground requires start")
  if (options.localOnly && command !== "doctor") throw new UsageError("--local-only requires doctor")
...
terminal to it from the command line; everything else — Apps, the Layout,
...
Commands:
  ...
  doctor         verify PTY helpers and the private directory

Options:
      --foreground run start in this terminal, owning local PTYs
      --local-only check doctor without requiring the Companion
      --name NAME  select an independent Instance (default: default)
  -h, --help       show this help
  -v, --version    print the version

Keys (attached Client only):
  ctrl-b d       detach this terminal, leaving the Instance alive
